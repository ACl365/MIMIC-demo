...
from src.features.build_features import _combine_features
...
class TestCombineFeatures(unittest.TestCase):
    ...
    @patch('src.features.build_features.os.path.exists')
    @patch('src.features.build_features.get_data_path')
    def test_combine_features_all_available(self, mock_get_data_path, mock_exists, mock_read_csv):
        """
        ...
        mock_get_data_path.side_effect = lambda data_type, dataset, config: {
            ("processed", "admission_data", self.config): "data/processed/admission_data.csv",
            ("processed", "base_path", self.config): "data/processed/"
        }[(data_type, dataset, config)]
    ...
    @patch('src.features.build_features.os.path.exists')
    @patch('src.features.build_features.get_data_path')
    def test_combine_features_missing_clinical(self, mock_get_data_path, mock_exists, mock_read_csv):
        """
        ...
        mock_exists.side_effect = lambda path: path != "data/processed/clinical_features.csv"
        ...
        self.assertIn("hadm_id", result.columns)
        self.assertIn("age", result.columns)
        self.assertNotIn("heart_rate_mean", result.columns)  # Clinical features should be missing
        self.assertIn("infectious", result.columns)
