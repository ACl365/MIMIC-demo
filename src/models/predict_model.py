...
def predict(
    ...
    output = pd.DataFrame({
        "subject_id": data["subject_id"] if "subject_id" in data.columns else np.nan,
...
def predict_all(
    ...
    # Readmission model
    try:
        readmission_output_path = os.path.join(output_dir, "readmission_predictions.csv")
    ...
    try:
        ...
        predictions["los"] = predict(
            data, "los", output_path=los_output_path
        )
    except FileNotFoundError:
...
def main() -> None:
    ...
    for model_type, preds in predictions.items():
        ...
        if model_type in ["readmission", "mortality"]:
            ...
            logger.info(f"  Positive predictions: {positive_count} ({positive_count / len(preds) * 100:.2f}%)")
        elif model_type == "los":
