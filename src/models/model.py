...
from typing import Dict, List, Optional, Tuple, Union, Any # Added Union, Any
...
def _reshape_data_to_admission_level(
    data: pd.DataFrame, id_cols: List[str], target_cols: List[str], logger_instance: logging.Logger
) -> pd.DataFrame:
    """
    ...
    if (
        ...
        and "0" in data.columns # Assuming '0' holds the value after some upstream processing
...
class BaseModel(ABC):
    ...
    def __init__(
        ...
        self.logger = logger
        self.model: Optional[Union[BaseEstimator, nn.Module]] = None # More specific type hint
    ...
    @abstractmethod
    def train(
        self, X_train: Union[pd.DataFrame, Dict], y_train: pd.Series, algorithm: Optional[str] = None
    ...
    @abstractmethod
    def evaluate(self, X_test: Union[pd.DataFrame, Dict], y_test: pd.Series) -> Dict[str, float]:
        """
    ...
    def fit(
        self, data: pd.DataFrame, algorithm: Optional[str] = None, test_size: float = 0.2
    ) -> Dict[str, float]:
        """
        ...
        if not isinstance(X, pd.DataFrame):
             raise TypeError(f"BaseModel.fit expects preprocess to return a DataFrame for X, but got {type(X)}. Override fit for different preprocess outputs.")
        if y is None:
            raise ValueError(
        ...
        if y.nunique() > 1 and len(y.value_counts()) > 1:
             ...
             n_splits_needed = self.cv_folds if self.hyperparameter_tuning else 2 # Need at least 2 for basic split
             # Also check if test_size * total_samples is large enough for stratification
             min_test_samples = max(n_splits_needed, int(np.ceil(test_size * len(y)))) # Heuristic
             if min_samples_per_class >= min_test_samples:
                 stratify_cond = y
             else:
                 self.logger.warning(f"Cannot stratify: Minimum samples per class ({min_samples_per_class}) might be too few for test size ({test_size}) and CV folds ({n_splits_needed}).")
    ...
    def predict(self, data: pd.DataFrame) -> np.ndarray:
        ...
        if not isinstance(X, pd.DataFrame):
             raise TypeError(f"BaseModel.predict expects preprocess to return a DataFrame for X, but got {type(X)}. Override predict for different preprocess outputs.")
    ...
    def load(cls, path: str) -> "BaseModel":
        ...
        try:
            ...
            if (
                ...
                model_instance.sequence_feature_names = checkpoint.get("sequence_feature_names") # Load specific feature lists
                ...
                model_instance.model_architecture_params = arch_params # Store loaded params


                # Basic checks for loaded components
                if model_instance.model is None: raise ValueError("Model state_dict loaded but model object is None.")
                if model_instance.sequence_scaler is None: logger.warning("Sequence scaler not found in loaded model data.")
                if model_instance.static_scaler is None: logger.warning("Static scaler not found in loaded model data.")
        ...
        try:
            ...
            # Add other model types here if needed
                ...
                try:
                    ...
                    if cls is BaseModel:
                         raise ValueError(f"Cannot directly instantiate abstract BaseModel for unknown type '{model_type}'")
                    model_instance = cls(model_type=model_type, config=config)
                except TypeError: # Handle case where cls might be BaseModel (ABC) or other instantiation issues
                     raise ValueError(f"Could not instantiate class '{cls.__name__}' for unknown model type '{model_type}'")
            ...
            if model_instance.scaler is None:
                 logger.warning(f"Scaler not found in pickle file at {path}. Using default StandardScaler.")
    ...
    def get_feature_importance(self, top_n: int = 20) -> pd.DataFrame:
        ...
        if importances is not None:
            ...
            if len(importances) != len(self.feature_names):
                 self.logger.error(f"Mismatch between number of importances ({len(importances)}) and feature names ({len(self.feature_names)}).")
    ...
    def get_shap_values(
        ...
        try:
            ...
            self.logger.error(f"Missing expected feature column for SHAP calculation: {e}")
            return None, None
        ...
        try:
            ...
            if hasattr(self.model, "predict_proba"):
                ...
                if background_data is not None:
                     ...
                     self.logger.warning("No background data provided for SHAP KernelExplainer. Results may be less reliable.")
                     # Create a dummy background if none provided (less ideal)
                     dummy_background = pd.DataFrame(np.zeros((1, len(self.feature_names))), columns=self.feature_names)
                 ...
                 self.logger.error("Model has neither predict_proba nor predict method for SHAP.")
                 return None, None
...
class ReadmissionModel(BaseModel):
    ...
    def __init__(self, config: Optional[Dict] = None, random_state: int = 42) -> None:
        ...
        if self.target is None: # Set default target if not in config
             self.logger.warning("Readmission target not specified in config, defaulting to 'readmission_30day'.")
    ...
    def preprocess(
        ...
        """
        self.logger.info(f"Preprocessing data for {'prediction' if for_prediction else 'training'}...")
        ...
        if not is_reshaped:
            ...
            target_cols = [self.target] if self.target and self.target in data.columns else []
            try:
                data = _reshape_data_to_admission_level(data, id_cols, target_cols, self.logger)
                 ...
                 return pd.DataFrame(), None if for_prediction else (pd.DataFrame(), pd.Series(dtype=float))
        else:
             self.logger.info("Data is already one row per admission. Skipping reshaping.")
        ...
        if missing_expected:
            self.logger.warning(f"Expected features missing from data after reshaping: {missing_expected}. They will be excluded.")

        if not available_features:
             self.logger.error("No configured features found in the data after reshaping.")
             return pd.DataFrame(), None if for_prediction else (pd.DataFrame(), pd.Series(dtype=float))
        ...
        for col in X.select_dtypes(include=np.number).columns:
            if X[col].isnull().any():
                ...
                self.logger.debug(f"Imputed missing numeric values in '{col}' with median ({median_val}).")

        for col in X.select_dtypes(include='object').columns:
             if X[col].isnull().any():
                 mode_val = X[col].mode()[0] if not X[col].mode().empty else "Unknown"
                 X[col] = X[col].fillna(mode_val)
                 self.logger.debug(f"Imputed missing categorical values in '{col}' with mode ('{mode_val}').")
            ...
            model = RandomForestClassifier(
             ...
             if class_weight == "balanced":
                 ...
                 if 1 in counts and 0 in counts and counts[1] > 0:
                     ...
                     self.logger.warning("Cannot calculate scale_pos_weight for XGBoost, using default.")
             ...
             if class_weight == "balanced":
                 ...
                 if 1 in counts and 0 in counts and counts[1] > 0:
                     ...
                     self.logger.warning("Cannot calculate scale_pos_weight for LightGBM, using default.")
        ...
        if self.hyperparameter_tuning:
            ...
            if algorithm == "logistic_regression":
                ...
                self.logger.info(f"Best parameters found: {grid_search.best_params_}")
                 ...
                 self.logger.warning(f"Hyperparameter tuning not implemented for {algorithm}. Using default parameters.")
                 model.fit(X_train, y_train) # Fit with default params if tuning not implemented
        else:
    ...
    def evaluate(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "roc_auc": roc_auc_score(y_test, y_prob) if hasattr(self.model, "predict_proba") else np.nan,
            "pr_auc": average_precision_score(y_test, y_prob) if hasattr(self.model, "predict_proba") else np.nan,
        }
...
class MortalityModel(BaseModel):
    ...
    def __init__(self, config: Optional[Dict] = None, random_state: int = 42) -> None:
        ...
        if self.target is None: # Set default target if not in config
             self.logger.warning("Mortality target not specified in config, defaulting to 'hospital_death'.")
    ...
    def preprocess(
        ...
        """
        self.logger.info(f"Preprocessing data for mortality {'prediction' if for_prediction else 'training'}...")
        # Use ReadmissionModel's preprocessing logic for now
        temp_model = ReadmissionModel(config=self.config, random_state=self.random_state)
        temp_model.target = self.target # Ensure correct target is used
        temp_model.model_config = self.model_config # Ensure correct config section is used
...
class LengthOfStayModel(BaseModel):
    ...
    def __init__(self, config: Optional[Dict] = None, random_state: int = 42) -> None:
        ...
        if self.target is None: # Set default target if not in config
             self.logger.warning("Length of Stay target not specified in config, defaulting to 'los_days'.")
    ...
    def preprocess(
        ...
        Returns:
            Tuple[pd.DataFrame, Optional[pd.Series]]: Preprocessed features (X) and target (y).
        """
        self.logger.info(f"Preprocessing data for LOS {'prediction' if for_prediction else 'training'}...")
        ...
        if not for_prediction:
            if self.target and self.target in data.columns:
                ...
                if self.model_config.get("log_transform_target", False):
                     y = np.log1p(y)
                     self.logger.info(f"Log-transformed target variable '{self.target}'.")
                self.logger.info(f"Target variable '{self.target}' extracted.")
            else:
                self.logger.error(
                    f"Target variable '{self.target}' not found in data columns: {data.columns.tolist()}"
    ...
    def evaluate(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict[str, float]:
        ...
        if self.model_config.get("log_transform_target", False):
             self.logger.info("Applying inverse log transformation (expm1) to predictions and target for evaluation.")
             y_pred = np.expm1(y_pred)
             y_test = np.expm1(y_test) # Also inverse transform test set for correct metric calculation
...
class TemporalReadmissionModel(BaseModel):
    ...
    def __init__(self, config: Optional[Dict] = None, random_state: int = 42) -> None:
        ...
        if self.target is None:
             self.logger.warning("Temporal Readmission target not specified, defaulting to 'readmission_30day'.")
             self.target = "readmission_30day"
        ...
        self.model_architecture_params: Optional[Dict] = None


    def _pad_sequence(self, seq: List[Tuple[float, float]], max_len: int, value: float = 0.0) -> List[Tuple[float, float]]:
        """
    ...
    def preprocess(
        ...
        Returns:
            Tuple[Dict[str, Any], Optional[pd.Series]]:
                ...
                - y (Optional[pd.Series]): Series containing the target variable (integer type)
                                            if not for_prediction, otherwise None.
        """
        self.logger.info(f"Preprocessing data for temporal {'prediction' if for_prediction else 'training'}...")

        # Ensure data is one row per admission (should be done before calling this)
        id_cols = ["subject_id", "hadm_id"]
        if data.duplicated(subset=id_cols).any():
             self.logger.warning("Input data contains multiple rows per admission. Ensure data is pre-aggregated.")
             ...
             try:
                 ...
                 self.logger.error(f"Error during data reshaping: {e}", exc_info=True)
                 return {}, None if for_prediction else ({}, pd.Series(dtype=float))
        ...
        if missing_static:
             self.logger.warning(f"Configured static features not found in data: {missing_static}")

        self.logger.info(f"Identified {len(sequence_cols)} sequence features.")
        self.logger.info(f"Identified {len(static_cols)} static features: {static_cols}")
        ...
        self.feature_names = self.sequence_feature_names + self.static_feature_names # Combined list for compatibility
        ...
        for col in self.sequence_feature_names:
            ...
            def _eval_seq(x):
                ...
                try: return eval(x) if isinstance(x, str) else []
                except: return []
            ...
            for i, seq in enumerate(processed_sequences[col]):
                 ...
                 if seq_len > 0:
                     ...
                     try:
                         ...
                         self.logger.warning(f"Error processing sequence for {col}, admission index {i}: {e}. Sequence: {seq}")
        ...
        if num_sequence_features > 0:
            value_data_to_scale = np.zeros((num_admissions * max_len, num_sequence_features))
            ...
            if not for_prediction:
                ...
                scaled_value_data = self.sequence_scaler.fit_transform(value_data_to_scale)
            else:
                if not hasattr(self.sequence_scaler, 'mean_'): # Check if scaler is fitted
                    self.logger.error("Sequence scaler has not been fitted. Cannot preprocess for prediction.")
                    return {}, None
            ...
            scaled_value_data = scaled_value_data.reshape(num_admissions, max_len, num_sequence_features)
            for idx, col in enumerate(self.sequence_feature_names):
        ...
        if static_cols:
             ...
             for col in static_features_df.select_dtypes(include=np.number).columns:
                 if static_features_df[col].isnull().any():
                     ...
                     self.logger.debug(f"Imputed missing static numeric values in '{col}' with median ({median_val}).")
             # Scale static features
             if not for_prediction:
                 self.logger.info("Fitting static feature scaler...")
                 static_features_scaled = self.static_scaler.fit_transform(static_features_df)
             else:
                 if not hasattr(self.static_scaler, 'mean_'):
                     self.logger.error("Static feature scaler has not been fitted. Cannot preprocess for prediction.")
                     return {}, None
                 self.logger.info("Transforming static features with fitted scaler...")
                 static_features_scaled = self.static_scaler.transform(static_features_df)

             static_features_df = pd.DataFrame(static_features_scaled, columns=static_cols, index=static_features_df.index)
        else:
        ...
        actual_lengths = all_sequence_lengths[0] if all_sequence_lengths else [0] * num_admissions
    ...
    def train(
        self, X_train: Dict[str, Any], y_train: pd.Series, algorithm: Optional[str] = None # Algorithm ignored for now
    ) -> nn.Module:
        """
        ...
        """
        self.logger.info(f"Training TimeAwarePatientLSTM model for {self.model_type}...")

        # --- Model Initialization ---
        # Determine input sizes based on preprocessed data
        num_sequence_features = len(self.sequence_feature_names) if self.sequence_feature_names else 0
        ...
        use_time_features = self.model_config.get("use_time_features", True) # Use time intervals by default
        ...
        self.model_architecture_params = {
             ...
             "dropout": dropout,
             "use_time_features": use_time_features,
             "device": str(self.device) # Store device as string for JSON compatibility if needed later
        }


        # --- Dataset and DataLoader ---
        train_dataset = TemporalEHRDataset(X_train, y_train.values) # Pass numpy array for target
    ...
    def _collate_fn(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        static_features_tensor = torch.tensor(np.array(static_features), dtype=torch.float32)
        targets_tensor = torch.tensor(targets, dtype=torch.long) # Assuming integer targets for classification
    ...
    def evaluate(
        self, X_test: Dict[str, Any], y_test: pd.Series
    ) -> Dict[str, float]:
        """
        ...
        test_dataset = TemporalEHRDataset(X_test, y_test.values)
        batch_size = self.model_config.get("batch_size", 64) # Use same batch size or a different one for eval
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=self._collate_fn)
    ...
    def predict(
        ...
        if not X_processed or not X_processed.get("admission_ids", pd.DataFrame()).shape[0] > 0:
             self.logger.error("Preprocessing failed or resulted in empty data during prediction.")
        ...
        predict_loader = DataLoader(predict_dataset, batch_size=predict_batch_size, shuffle=False, collate_fn=self._collate_fn)
    ...
    def save(self, path: str) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error saving temporal model to {path}: {e}", exc_info=True)
        finally:
    ...
    def load(cls, path: str) -> "TemporalReadmissionModel":
        ...
        try:
            ...
            if model_type != "temporal_readmission":
                 raise ValueError(f"Model type in file ({model_type}) does not match expected 'temporal_readmission'")

            # Instantiate the model class
            model_instance = cls(config=config) # cls refers to TemporalReadmissionModel
            ...
            # Load metadata
            model_instance.feature_names = checkpoint.get("feature_names")
            model_instance.sequence_feature_names = checkpoint.get("sequence_feature_names")
            model_instance.static_feature_names = checkpoint.get("static_feature_names")
            model_instance.sequence_scaler = checkpoint.get("sequence_scaler")
            model_instance.static_scaler = checkpoint.get("static_scaler")
            model_instance.target = checkpoint.get("target")
            ...
            logger.info(f"Loaded Temporal model '{model_instance.model_type}' from {path} onto {device}")
            return model_instance
            ...
            logger.error(f"Error loading temporal model from {path}: {e}", exc_info=True)
            raise
    ...
    def get_feature_importance(self, top_n: int = 20) -> pd.DataFrame:
        ...
        self.logger.warning("Standard feature importance is not directly applicable to LSTM models.")
        return pd.DataFrame(columns=["feature", "importance"])

    def get_shap_values(
        self, X: Dict, background_data: Optional[Dict] = None
    ) -> Tuple[Optional[np.ndarray], Optional[Any]]:
        """SHAP values for LSTMs require specialized explainers (e.g., DeepExplainer) and are not implemented here."""
        self.logger.warning("SHAP value calculation for TimeAwarePatientLSTM requires specific handling and is not implemented by default.")
