...
from src.models.model import (
    ...
    MortalityModel,
    ReadmissionModel,
    TemporalReadmissionModel, # <-- Add import
)
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        if model.model is not None and model.feature_names is not None:
            try:
                ...
                input_example_df = data[model.feature_names] if model.feature_names else data
                input_example = input_example_df.head() if not input_example_df.empty else None
                ...
                logger.info(f"Logged model artifact to MLflow path: {model_type}")
                 ...
                 logger.error(f"Failed to log model with mlflow.sklearn: {e}", exc_info=True)
                 logger.warning("Attempting to save raw pickle as fallback.")
                 # Fallback: Log the saved pickle file
                 if save_path is None:
                     save_path = os.path.join(
                         get_project_root(), "models", f"{model_type}_model_{run_id[:8]}.pkl" # Include run_id part
                     )
                 try:
                     ...
                     logger.info(f"Logged model pickle artifact: {save_path}")
                      ...
                      logger.error(f"Failed to save model pickle locally: {save_e}", exc_info=True)
        ...
        if model.model is not None and model.feature_names is not None:
            try:
                ...
                logger.info(f"Logged model artifact to MLflow path: {model_type}")
                 ...
                 if save_path is None:
                     save_path = os.path.join(
                         get_project_root(), "models", f"{model_type}_model_{run_id[:8]}.pkl"
                     )
                 try:
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        if model.model is not None: # Check if the internal PyTorch model exists
             try:
                 ...
                 if save_path is None:
                     save_path = os.path.join(
                         get_project_root(), "models", f"{model_type}_model_{run_id[:8]}" # Dir for PyTorch model files
                     )
                 model.save(save_path) # Save locally first (might save multiple files)
                 mlflow.log_artifacts(save_path, artifact_path=model_type) # Log the directory
                 logger.info(f"Logged model artifacts to MLflow path: {model_type} from dir: {save_path}")
             except Exception as e:
                  logger.error(f"Failed to save or log temporal model artifact: {e}", exc_info=True)
        else:
             logger.warning("Trained temporal model object not found. Cannot log model artifact via train script.")
...
def train_models(
    ...
    try:
        ...
        logger.error(f"Combined features file not found at {data_path}. Cannot train models.")
        return {}
    except Exception as e:
    ...
    try:
        ...
        logger.error(f"Failed to set MLflow experiment '{experiment_name}': {e}", exc_info=True)
        # Decide if training should proceed without MLflow or stop
        logger.warning("Proceeding with training without MLflow tracking due to setup error.")
    ...
    if model_type: # Train only specified model type
        if model_type in models_to_train:
             ...
             if model_type == "readmission":
                 ...
                 metrics["temporal_readmission"] = train_temporal_readmission_model(data, config)
             # Add elif for other model types like 'temporal_readmission' if needed
             else:
                  logger.warning(f"Training function for model type '{model_type}' not implemented.")
        else:
             logger.error(f"Specified model type '{model_type}' not found in configuration.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        type=str,
        choices=["readmission", "mortality", "los", "temporal_readmission"], # <-- Add temporal choice
        default=None,
        help="Type of model to train (default: all)",
    )
    parser.add_argument("--algorithm", type=str, default=None, help="Specific algorithm to use (optional)")
