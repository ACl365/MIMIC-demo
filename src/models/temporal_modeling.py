...
class TemporalEHRDataset(Dataset):
    ...
    def __init__(
        self,
        processed_data: Dict[str, Any], # Expects the dict from TemporalReadmissionModel.preprocess
        ...
        self.hadm_ids = self.admission_ids_df['hadm_id'].tolist() # Assuming hadm_id is the key identifier
        ...
        self.sequence_feature_names = sorted([
            k.replace('_values_scaled', '') for k in self.sequences if k.endswith('_values_scaled')
        ])
        ...
        if len(self.hadm_ids) != len(self.lengths):
             raise ValueError("Mismatch between number of admission IDs and sequence lengths.")
        if len(self.hadm_ids) != len(self.static_features):
             raise ValueError("Mismatch between number of admission IDs and static features.")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for base_name in self.sequence_feature_names:
            ...
            if values_key in self.sequences:
                 ...
                 seq_vals = self.sequences[values_key][idx, :seq_len] # Get unpadded sequence
                 ...
                 sequence_values_list.append(np.zeros(seq_len)) # Example: fill with zeros
            ...
            if not sequence_intervals_list and seq_len > 0: # Only grab intervals once
                 ...
                 if intervals_key in self.sequences:
                      ...
                      sequence_intervals_list = np.zeros(seq_len) # Example: fill with zeros


        # Stack features along the feature dimension
        sequence_values_array = np.stack(sequence_values_list, axis=-1) if sequence_values_list else np.zeros((seq_len, 0))
        # Ensure intervals is a 1D array
        sequence_intervals_array = np.array(sequence_intervals_list) if sequence_intervals_list is not None else np.zeros(seq_len)
...
class TimeEncoder(nn.Module):
    ...
    def forward(self, time_intervals: torch.Tensor) -> torch.Tensor:
        ...
        if time_intervals.ndim == 2:
            time_intervals = time_intervals.unsqueeze(-1) # Add feature dim: [batch_size, seq_len, 1]

        time_encoding = self.time_embed(time_intervals) # Output: [batch_size, seq_len, embed_dim]
...
class TimeAwarePatientLSTM(nn.Module):
    ...
    def forward(
        ...
        if self.use_time_features and self.time_encoder is not None:
            time_encoding = self.time_encoder(time_intervals) # [batch_size, max_seq_len, time_embed_dim]
        ...
        packed_input = nn.utils.rnn.pack_padded_sequence(
            x_seq_combined, lengths, batch_first=True, enforce_sorted=False # Already sorted in collate_fn
        )
        ...
        attention_logits = self.attention(lstm_out)  # [batch_size, max_actual_seq_len, 1]
        # Create mask based on actual lengths to ignore padding in softmax
        mask = torch.arange(lstm_out.size(1))[None, :].to(self.device) < torch.tensor(lengths)[:, None].to(self.device)
        attention_logits.masked_fill_(~mask.unsqueeze(-1), -float('inf')) # Mask padding tokens

        attention_weights = torch.softmax(attention_logits, dim=1)  # [batch_size, max_actual_seq_len, 1]
        ...
        if x_static.size(0) != batch_size:
             ...
             raise ValueError(f"Batch size mismatch between context ({context.size(0)}) and static features ({x_static.size(0)})")

        combined_features = torch.cat([context, x_static], dim=1)  # [batch_size, hidden_dim + num_static_features]
...
def get_attention_weights(
    ...
    with torch.no_grad():
        ...
        seq_int_tensor = torch.FloatTensor(sequence_intervals).unsqueeze(0).to(device) # Add feature dim later if needed by TimeEncoder
        ...
        try:
            ...
            mask = torch.arange(lstm_out.size(1))[None, :].to(device) < torch.tensor(lengths_list)[:, None].to(device)
