...
def build_features(config: Optional[Dict] = None) -> None:
    ...
    if config["features"]["demographic"]["include"]:
        ...
        demographic_output_path = os.path.join(
            get_data_path("processed", "base_path", config), "demographic_features.csv"
        )
    ...
    if config["features"]["diagnoses"]["include"]:
        ...
        diagnosis_output_path = os.path.join(
            get_data_path("processed", "base_path", config), "diagnosis_features.csv"
        )
...
def _combine_features(config: Dict) -> pd.DataFrame:
    ...
    if (
        os.path.exists(demographic_path)
        and config["features"]["demographic"]["include"]
    ):
        ...
        if is_debug_enabled():
            logger.debug(
                f"Demographic features columns: {demographic_features.columns.tolist()}"
            )
        ...
        if (
            len(demographic_features) > 0
            and "subject_id" in demographic_features.columns
            ...
            if is_debug_enabled():
                logger.debug(
                    f"df_to_merge columns before drop: {df_to_merge.columns.tolist()}"
                )
                logger.debug(
                    f"df_to_merge columns (keeping IDs): {df_to_merge.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features columns before merge: {combined_features.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features shape before merge: {combined_features.shape}"
                )

            combined_features = pd.merge(
                combined_features, df_to_merge, on=["subject_id", "hadm_id"], how="left"
            )

            if is_debug_enabled():
                logger.debug(
                    f"Combined features columns after merge: {combined_features.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features shape after merge: {combined_features.shape}"
                )
            else:
            ...
            if len(demographic_features) > 0:
                logger.warning(
                    f"Available columns: {demographic_features.columns.tolist()}"
                )
    ...
    if os.path.exists(clinical_path) and (
        ...
        if is_debug_enabled():
            logger.debug(
                f"Clinical features columns: {clinical_features.columns.tolist()}"
            )
        ...
        if is_debug_enabled():
            logger.debug(
                f"Clinical features columns after lowercase conversion: {clinical_features.columns.tolist()}"
            )
        ...
        if "subject_id" in clinical_features.columns:
            if is_debug_enabled():
                ...
                logger.debug(
                    f"Clinical features columns before merge: {clinical_features.columns.tolist()}"
                )
                logger.debug(f"Clinical features shape: {clinical_features.shape}")
                logger.debug(
                    f"Combined features columns before clinical merge: {combined_features.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features shape before clinical merge: {combined_features.shape}"
                )
            ...
            if is_debug_enabled():
                logger.debug(
                    f"Combined features columns after clinical merge: {combined_features.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features shape after clinical merge: {combined_features.shape}"
                )
            else:
    ...
    if os.path.exists(diagnosis_path) and config["features"]["diagnoses"]["include"]:
        ...
        # Check if the dataframe has the required columns
        if (
            len(diagnosis_features) > 0
            and "subject_id" in diagnosis_features.columns
            ...
            if is_debug_enabled():
                logger.debug(
                    f"Diagnosis df_to_merge columns (keeping IDs): {df_to_merge.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features columns before diagnosis merge: {combined_features.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features shape before diagnosis merge: {combined_features.shape}"
                )
            ...
            if is_debug_enabled():
                logger.debug(
                    f"Combined features columns after diagnosis merge: {combined_features.columns.tolist()}"
                )
                logger.debug(
                    f"Combined features shape after diagnosis merge: {combined_features.shape}"
                )
            else:
    ...
    binary_cols = [
        ...
        if combined_features[col].dropna().isin([0, 1]).all()
    ]
    ...
    count_cols = [
        ...
        for col in numeric_cols
        if (
            col.endswith("_count") or col.startswith("count_") or "count" in col.lower()
        )
    ...
    clinical_cols = [
        col
        for col in numeric_cols
        if (col not in id_cols + target_cols + binary_cols + count_cols)
        and (
            "_mean" in col
            or "_min" in col
            or "_max" in col
    ...
    other_numeric_cols = [
        col
        for col in numeric_cols
        if col not in id_cols + target_cols + binary_cols + count_cols + clinical_cols
    ]
    ...
    remaining_na_cols = combined_features.columns[
        combined_features.isna().any()
    ].tolist()
    if remaining_na_cols:
        logger.info(
            f"Filling {len(remaining_na_cols)} remaining columns with appropriate values"
        )
        for col in remaining_na_cols:
            # Ensure we don't try to fill sequence columns with 'Unknown'
            if col not in numeric_cols and not col.endswith("_sequence_data"):
                combined_features[col] = combined_features[col].fillna("Unknown")
            elif col.endswith("_sequence_data"):
                # Sequence columns should already be filled with ([], []) by the extractor
                # If any NaNs remain somehow, fill them appropriately
                if combined_features[col].isna().any():
                    logger.warning(
                        f"Found unexpected NaNs in sequence column {col}. Filling with empty sequence tuple ([], [])."
                    )
