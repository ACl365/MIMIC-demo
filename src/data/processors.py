...
class PatientProcessor(BaseProcessor):
    ...
    def process(self) -> pd.DataFrame:
        ...
        try:
            ...
            mimic4_patients_path = os.path.join(
                mimic4_path, "patients.csv", "patients.csv"
            )
            ...
            if os.path.exists(mimic4_patients_path):
                ...
                self.logger.warning(
                    "MIMIC-IV patient data not found, using only MIMIC-III data"
                )
    ...
    def _process_mimic3_patients(self, patients: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.warning(
                "Error calculating age from dates. Using default values."
            )
...
class AdmissionProcessor(BaseProcessor):
    ...
    def process(self) -> pd.DataFrame:
        ...
        try:
            ...
            mimic4_admissions_path = os.path.join(
                mimic4_path, "admissions.csv", "admissions.csv"
            )
            ...
            if os.path.exists(mimic4_admissions_path):
                ...
                self.logger.warning(
                    "MIMIC-IV admission data not found, using only MIMIC-III data"
                )
    ...
    def _calculate_derived_features(self, admissions: pd.DataFrame) -> pd.DataFrame:
        ...
        if "hospital_expire_flag" in admissions.columns:
            ...
            admissions["hospital_death"] = ~admissions[
                "deathtime"
            ].isna()  # Example fallback
            self.logger.warning(
                "'hospital_expire_flag' not found. Inferring 'hospital_death' from 'deathtime'."
            )
    ...
    def _identify_readmissions(self, admissions: pd.DataFrame) -> pd.DataFrame:
        ...
        admissions_shifted.loc[valid_rows, "days_to_readmission"] = (
            ...
            - admissions_shifted.loc[valid_rows, "dischtime"]
        ).dt.total_seconds() / (24 * 60 * 60)

        # Flag readmissions within time windows
        admissions_shifted.loc[
            valid_rows & (admissions_shifted["days_to_readmission"] <= 30),
            "readmission_30day",
        ] = True
        admissions_shifted.loc[
            valid_rows & (admissions_shifted["days_to_readmission"] <= 90),
            "readmission_90day",
        ] = True
    ...
    def process(self) -> pd.DataFrame:
        ...
        try:
            ...
            mimic4_icustays_path = os.path.join(
                mimic4_path, "icustays.csv", "icustays.csv"
            )
            ...
            if os.path.exists(mimic4_icustays_path):
                ...
                self.logger.warning(
                    "MIMIC-IV ICU stay data not found, using only MIMIC-III data"
                )
